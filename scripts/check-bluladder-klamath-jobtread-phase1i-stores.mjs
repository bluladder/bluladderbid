...
for (const forbidden of [
  ...
  if (contents.source?.includes(forbidden)) {
    errors.push(`store must keep side effects and secrets absent: ${forbidden}`);
  }
}
for (const phrase of [
...
if (productionImports.length) {
  errors.push(`Phase 1I stores became reachable from: ${productionImports.join(", ")}`);
}
...
console.log(
  "Klamath JobTread Phase 1I stores OK: direct attempt-one ownership, fail-closed ambiguity recovery, started-only terminal transitions, and read-only reconciliation remain dormant.",
);
