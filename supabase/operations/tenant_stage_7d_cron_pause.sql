...
-- Pause jobs 3, 5, and 6 without reading or rewriting their commands.
...
BEGIN
  ...
  IF matched <> 3 THEN
    RAISE EXCEPTION 'cron tuple precondition failed';
  END IF;

  PERFORM cron.alter_job(3, active := false);
  PERFORM cron.alter_job(5, active := false);
  PERFORM cron.alter_job(6, active := false);
  ...
  IF matched <> 3 THEN
    RAISE EXCEPTION 'cron pause postcondition failed';
  END IF;
END $stage7d$;
