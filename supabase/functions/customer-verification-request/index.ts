...
const GENERIC_OK = {
  status: "ok",
  message:
    "If that contact method is reachable, you will receive a code shortly.",
};

const validEmail = (raw: string | null) =>
  !!raw && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(raw);
...
serve(async (req) => {
  ...
  try {
    ...
    if (authority.status !== "resolved") return respond();
    ...
    const now = Date.now();
    const cooldownIso = new Date(now - cfg.per_phone_cooldown_seconds * 1000)
      .toISOString();
    ...
    if (hourError) return respond();
    if ((hourCount ?? 0) >= cfg.per_phone_max_per_hour) return respond();

    const { count: ipHourCount, error: ipHourError } = await supabase
      .from("customer_verification_challenges")
      .select("id", { count: "exact", head: true })
      .eq("organization_id", organizationId)
      .eq("ip_hash", ipHash)
      .gte("created_at", hourAgoIso);
    if (ipHourError) return respond();
    ...
    const usableUntil = new Date(
      now + (cfg.otp_ttl_seconds + OTP_GRACE_SECONDS) * 1000,
    ).toISOString();
    ...
    if (optedOut || testSup.suppressed) {
      await supabase.from("customer_verification_challenges")
        .update({
          delivery_status: testSup.suppressed
    ...
    if (email && !phone) {
      const textBody =
        `Your BluLadder verification code is ${otp}. It expires in ${
          Math.round((cfg.otp_ttl_seconds + OTP_GRACE_SECONDS) / 60)
      ...
      await supabase.from("customer_verification_challenges").update({
        ...
        delivery_status: result.ok
          ? "accepted"
          : (result.failure?.category === "suppressed"
            ? "suppressed_email"
            : "delivery_failed"),
      }).eq("id", challenge.id);
