    ...
    const chain: any = {
      ...
      maybeSingle: async () => {
        if (table === "chat_conversations") {
          const row = rows.conversation ?? null;
          return {
            data: row &&
                !Object.prototype.hasOwnProperty.call(row, "organization_id")
...
Deno.test("getAvailableSlots: expected organization blocks cross-tenant conversation before provider call", async () => {
  ...
  assertEquals(res.status, "not_ready");
  assertEquals(res.detail, "organization_context_mismatch");
  assertEquals(fetcherCalled, false);
  assertEquals(
...
Deno.test("getAvailableSlots: an organization without the explicit DFW connector never reaches the provider", async () => {
  ...
  assertEquals(
    result.detail,
    "provider_connector_unavailable_for_organization",
  );
  assertEquals(fetcherCalled, false);
});

Deno.test("getAvailableSlots: a row-derived non-DFW organization cannot reach the DFW connector", async () => {
  const organizationId = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb";
  const { sb } = makeSpySupabase({
    conversation: {
      id: "c1",
...
Deno.test("getAvailableSlots: canonically ready mixed quote exposes only its bookable portion without a duplicate manual-review veto", async () => {
  const { sb, log } = makeSpySupabase({
    conversation: {
      id: "c1",
      prospect_phone: "+14690000000",
  ...
  const mixedBookableReadiness = readyReadiness();
  mixedBookableReadiness.quote.manual_review_required = true;
  mixedBookableReadiness.quote.manual_review_reasons = [
    "separate service requires manual review",
  ];
  const res = await getAvailableSlots(
    sb as any,
    "c1",
    ...
    {
      readinessOverride: mixedBookableReadiness,
      fetcher,
      gateOverride: ALLOW_GATE,
    },
  );
...
Deno.test("getAvailableSlots: property, quote, and validated address must agree before provider dispatch", async () => {
  ...
  assertEquals(result.status, "not_ready");
  assertEquals(result.detail, "booking_address_lineage_mismatch");
  assertEquals(result.next_action, "verify_service_area");
  assertEquals(fetcherCalled, false);
});

Deno.test("getAvailableSlots: provider intervals shorter than canonical duration are never offered", async () => {
    ...
    fetcher: async () => ({
      ...
      json: {
        recommendations: [{
          ...
          endTime: "2026-07-27T10:00:00-05:00",
          displayTime: "9:00 AM",
        }],
      },
    }),
  ...
  assertEquals(result.status, "no_slots");
