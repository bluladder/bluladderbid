...
export interface ParsedAdapterRequest {
  ...
  stream: boolean;
  /**
   * Session ID for correlating this HTTP turn with an in-progress conversation.
   * An authenticated provider call ID is authoritative when present. Generic
   * header/user correlators may only be used when no provider ID exists and
  ...
  /** Best-effort E.164 caller ID (ANI) extracted from provider metadata.
   *  Never trusted for authorization by itself — used only for caller-ID
   *  confirmation UX and rollout allowlisting. */
  callerIdE164: string | null;
  /** Raw parsed body — retained so downstream adapters (rollout gate) can
   *  inspect provider metadata without re-parsing. Never forwarded to the
   *  language model. */
...
export type AdapterRequestError =
  ...
  | { kind: "invalid_session_identifier" }
  | { kind: "conflicting_session_identifiers" };

const SESSION_IDENTIFIER_PATTERN = /^[A-Za-z0-9._:+@-]{1,160}$/;

function normalizeSessionIdentifier(value: unknown): string {
  return typeof value === "string" ? value.trim() : "";
}

function isValidSessionIdentifier(value: string): boolean {
  return SESSION_IDENTIFIER_PATTERN.test(value);
}
...
export async function parseAdapterRequest(
  ...
  const bodyCallId = normalizeSessionIdentifier(
    body?.call?.id ||
      body?.metadata?.call?.id ||
      body?.metadata?.callId,
  );
  const identifiers = [headerSession, userField, bodyCallId].filter(Boolean);
  if (identifiers.some((value) => !isValidSessionIdentifier(value))) {
    return { ok: false, error: { kind: "invalid_session_identifier" } };
  }
  ...
  const suppliedSession = providerSession || headerSession || userField;
  const sessionIdIsSynthetic = !suppliedSession;
  const sessionId = suppliedSession || `synthetic-${crypto.randomUUID()}`;

  // Caller ID (ANI) — best-effort extraction from known Vapi shapes.
  const callerCandidates: unknown[] = [
    body?.call?.customer?.number,
    body?.call?.customer?.phoneNumber,
    body?.call?.from,
    body?.call?.fromNumber,
    body?.customer?.number,
    body?.metadata?.callerId,
    body?.metadata?.from,
