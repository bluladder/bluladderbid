...
Deno.test("Klamath manifest preserves the approved Realtime pipeline", () => {
  const manifest = buildKlamathVoiceRealtimeManifest({ serverEventsUrl });
  assertEquals(manifest.model.provider, "openai");
  assertEquals(manifest.model.model, VOICE_REALTIME_MVP_MODEL);
  assertEquals(manifest.voice, {
    provider: "openai",
    voiceId: VOICE_REALTIME_MVP_VOICE,
  });
...
Deno.test("Klamath manifest preserves duration, privacy, and analysis gates", () => {
  const manifest = buildKlamathVoiceRealtimeManifest({ serverEventsUrl });
  assertEquals(manifest.duration.maxDurationSeconds, 900);
  assertEquals(
    manifest.duration.timeElapsedHooks.map((hook) => hook.seconds),
    [780, 870],
  );
  assertEquals(
