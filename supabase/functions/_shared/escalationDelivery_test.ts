...
Deno.test("failed delivery produces the fallback-number language", () => {
  const msg = customerEscalationMessage(
    "delivery_failed",
    "high",
    "(469) 747-2877",
  );
...
Deno.test("urgent language only when severity is urgent", () => {
  const urgent = customerEscalationMessage(
    "sms_sent",
    "urgent",
    "(469) 747-2877",
  );
...
Deno.test("SMS escalation stays in the text conversation without a circular call CTA", () => {
  assertEquals(
    customerEscalationMessage(
      "sms_sent",
      "normal",
  ...
  for (
    ...
    const message = customerEscalationMessage(
      state,
      "normal",
      "(469) 747-2877",
      "sms",
    );
    assertEquals(/call|469|office/i.test(message), false);
    assertEquals(/reply here|follow up with you here/i.test(message), true);
  }
});
