-- Read-only postflight for split Klamath public call/text channels.
...
constraint_state AS (
  SELECT
    ...
    ) AS split_channel_constraint_count,
    count(*) FILTER (
      WHERE conname = 'organization_public_contacts_destination_check'
        ...
        AND position('sms' IN pg_get_constraintdef(oid)) > 0
        AND position('email' IN pg_get_constraintdef(oid)) > 0
    ) AS split_destination_constraint_count
  FROM pg_constraint
...
SELECT jsonb_build_object(
  ...
  'split_channel_constraint_count',
    constraint_state.split_channel_constraint_count,
  'split_destination_constraint_count',
    constraint_state.split_destination_constraint_count,
...
) AS split_public_contact_postflight
FROM target_state, row_state, constraint_state, policy_state, grant_state,
