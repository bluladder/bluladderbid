-- Read-only postflight for a future BluLadder Klamath compliance-only switch.
-- The site may resolve and render compliance/contact surfaces, but no customer
-- workflow, pricing, provider, territory, or messaging runtime may be active.
...
WITH organization_state AS (
  SELECT
    ...
    ) AS exact_klamath_active_count
  FROM public.organizations
), site_state AS (
  SELECT count(*) AS exact_compliance_only_site_count
