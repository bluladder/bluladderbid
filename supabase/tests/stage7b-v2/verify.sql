...
-- Old-runtime/new-schema compatibility: before trusted provider mappings and
-- the dependent runtime are released, a new unscoped session and conversation
-- may still link to each other. The trigger must preserve null provenance, not
-- invent an organization or break the existing voice/chat path.
INSERT INTO public.quote_sessions(id, channel)
VALUES (
  'f0000000-0000-4000-8000-000000000013', 'voice'
) ON CONFLICT (id) DO NOTHING;
INSERT INTO public.chat_conversations(id, session_token, quote_session_id)
VALUES (
  '90000000-0000-4000-8000-000000000013', 'legacy-transition',
  'f0000000-0000-4000-8000-000000000013'
) ON CONFLICT (id) DO NOTHING;
DO $$
BEGIN
  IF (SELECT organization_id FROM public.quote_sessions
      WHERE id='f0000000-0000-4000-8000-000000000013') IS NOT NULL
     OR (SELECT organization_id FROM public.chat_conversations
         WHERE id='90000000-0000-4000-8000-000000000013') IS NOT NULL THEN
    RAISE EXCEPTION 'old-runtime compatibility invented tenant authority';
  END IF;
END
$$;
...
BEGIN
  ...
  END IF;
  UPDATE public.chat_conversations
  SET session_token='tenant-a-authorized-update'
  WHERE id='90000000-0000-4000-8000-000000000001';
  IF NOT FOUND THEN
    RAISE EXCEPTION 'tenant A authorized update was blocked';
  END IF;
END
$$;
ROLLBACK;
...
BEGIN
  ...
  VALUES (
    '90000000-0000-4000-8000-000000000097', 'tenant-a-insert',
    'b0000000-0000-4000-8000-000000000001'
  );
  BEGIN
    INSERT INTO public.chat_conversations(id, session_token, organization_id)
    VALUES (
      '90000000-0000-4000-8000-000000000098', 'tenant-b-insert',
      'b0000000-0000-4000-8000-000000000002'
    );
    RAISE EXCEPTION 'cross-tenant WITH CHECK unexpectedly succeeded';
...
-- service_role deliberately bypasses RLS. This proves the database does not
-- pretend otherwise; the later runtime PR must add explicit organization
-- predicates before it may be deployed.
BEGIN;
SET LOCAL ROLE service_role;
DO $$
BEGIN
  IF (SELECT count(*) FROM public.quote_sessions) <> 4
     OR (SELECT count(*) FROM public.chat_conversations) <> 4 THEN
    RAISE EXCEPTION 'service-role bypass expectation changed';
  END IF;
END
$$;
ROLLBACK;
...
DO $$
BEGIN
  IF (SELECT count(*) FROM public.quote_sessions) <> 4 THEN
