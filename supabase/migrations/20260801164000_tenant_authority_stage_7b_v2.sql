...
BEGIN
  IF EXISTS (
    ...
    CROSS JOIN LATERAL (
      SELECT count(DISTINCT candidate) AS organization_count
      FROM unnest(ARRAY[c.organization_id, p.organization_id, q.organization_id])
        candidate
      WHERE candidate IS NOT NULL
    ) evidence
    WHERE evidence.organization_count > 1
...
WITH evidence AS (
  ...
  CROSS JOIN LATERAL unnest(
    ARRAY[c.organization_id, p.organization_id, q.organization_id]
  ) candidate
  WHERE candidate IS NOT NULL
  GROUP BY qs.id
...
-- Every current Data API view is invoker-secured and read-only. Keep public
-- eligibility/technician reads while preventing accidental DML grants.
