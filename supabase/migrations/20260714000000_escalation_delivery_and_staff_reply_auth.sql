-- ============================================================================
-- Explicit escalation delivery states + per-channel audit, and a single-use,
-- short-lived staff-reply test-suppression authorization (never global).
-- ============================================================================
...
ALTER TABLE public.ai_escalations
  ALTER COLUMN alert_status SET DEFAULT 'created';
...
ALTER TABLE public.ai_escalations
  ...
  CHECK (alert_status IN (
    ...
    -- legacy values kept valid so historical rows never violate the constraint
    'pending','sent','no_recipient'
  ));

-- Per-channel status + provider audit for the internal alert.
...
-- Scoped to ONE conversation + ONE channel, short expiry, consumed exactly once.
-- This NEVER disables the permanent test-identity suppression globally.
...
-- Operations-admin RPC that issues ONE authorization. Requires an admin session
-- (auth.uid()); it cannot be called anonymously or by the anon key.
...
-- Atomically consume a valid, unexpired, unconsumed authorization for the
-- conversation+channel. Returns the consumed id or NULL if none is available.
-- SECURITY DEFINER so the service-role edge function can consume it.
