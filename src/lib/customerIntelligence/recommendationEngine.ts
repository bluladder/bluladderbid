...
export class OrganizationLineageError extends Error {
  constructor() {
    ...
    this.name = 'OrganizationLineageError';
  }
}

function assertLineage(expectedOrganizationId: string, actualOrganizationId: string): void {
...
export function scoreServiceRecommendation(args: {
  ...
  assertLineage(features.organizationId, model.organizationId);
  if (model.status !== 'approved') {
    throw new Error('model_unapproved');
  }

  if (features.archivedStatus === 'archived') {
    return {
      organizationId: features.organizationId,
      modelId: model.id,
      modelVersion: model.version,
      featureVersion: features.featureVersion,
      inputFingerprint: features.inputFingerprint,
      serviceKey,
      score: 0,
      confidence: 1,
...
export function rankRecommendations(args: {
  ...
  assertLineage(args.features.organizationId, args.model.organizationId);
  if (args.model.status !== 'approved') throw new Error('model_unapproved');
  if (args.features.archivedStatus === 'archived') return [];

  const eligibleServices = args.services.filter((serviceKey) => {
    const matches = args.catalog.filter((entry) =>
      entry.organizationId === args.features.organizationId
      && entry.serviceKey === serviceKey
      && entry.approvalStatus === 'approved'
      && (entry.quoteAvailability === 'enabled' || entry.planAvailability === 'enabled')
    );
...
export function outcomeLearningValue(outcome: LearningOutcome): number | null {
  if (outcome.archivedStatus === 'archived') return null;
...
export function proposeBoundedWeightUpdate(args: {
  ...
  for (const outcome of args.outcomes) {
    assertLineage(args.organizationId, outcome.organizationId);
    if (outcome.modelId !== args.modelId) throw new Error('model_lineage_mismatch');
  }
  ...
  for (const [feature, counts] of grouped) {
    const featureOutcomes = eligibleOutcomes.filter((outcome) => outcome.feature === feature);
    const positives = featureOutcomes.filter((outcome) => outcome.value >= 0.5).length;
    if (
      counts.total < args.bounds.minimumOutcomes
      || positives < args.bounds.minimumPositiveOutcomes
    ) continue;
  ...
  return {
    eligible: Object.keys(summary).length > 0,
    proposedWeights,
    summary,
  };
}
